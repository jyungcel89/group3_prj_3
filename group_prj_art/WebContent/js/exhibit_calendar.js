...
(function ($) {
	...
	$.fn.calendar = function (options) {
		...
		_this.delegate('tbody td', 'click', function () {
			...
			if(_this.data('date') != null){
				var selectDate=_this.data('date');
				$("input[name=selectDate]").val(selectDate);
				//var option="width=660, height=280, top=200, left=200, resizable=0, scrollbars=yes, status=no";
				var option="width=10, height=10, top=200, left=200, resizable=0, scrollbars=yes, status=no";
				var obj=document.calFrm;
				var popupCal = window.open('about:blank','popup_calendar',option);
			    obj.target ="popup_calendar";
			    obj.method ="post";
			    obj.submit();
			}//end if
	...
	$.fn.datePicker = function () {
		...
		picker.click(function () {
			...
			alert(_this.val(picker.getCurrentDate()));
			return false;
		});
	...
	$(window).load(function () {
		$('.jquery-ex-calendar').each(function () {
			$(this).calendar();
		});//each
	});//window
}($));
